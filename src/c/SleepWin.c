...
static void window_load(Window *window) {
  ...
  time_end_sleep = persist_exists(TIMER_SLEEP_KEY) ? persist_read_int(TIMER_SLEEP_KEY)  : TIMER_SLEEP_DEFAULT;
...
static void window_unload(Window *sleep_main_window) {
  ...
  if (!pause){
    persist_write_int(TIMER_SLEEP_KEY, time_end_sleep = time_begin_sleep);
  }
